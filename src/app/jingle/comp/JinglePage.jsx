...
// ── Video temporarily hidden — set VIDEO_SRC & POSTER back to enable video ──
...
// const POSTER    = 'https://res.cloudinary.com/dgifa4wgb/video/upload/so_17/v1772943166/f4d8fc7a-c497-4d75-83d8-52d71ee41ec7_t9a4lo.jpg'
const VIDEO_SRC = ''
const POSTER = ''
...
export default function JinglePage() {
  ...
  }, [hasVideo, playing, showControls])

  function handleMouseMove() {
    if (hasVideo) setShowControls(true)
  }
  ...
  return (
    <div className={`${styles.root} ${entered ? styles.entered : ''}`} onMouseMove={handleMouseMove}>
      ...
      {/* ── Main theatre ── */}
      <main className={styles.theatre} onClick={hasVideo ? togglePlay : undefined}>
        ...
        {hasVideo ? (
          ...
          // No video, no poster — brand gradient backdrop instead
          <div
            className={styles.video}
            style={{
              background: 'radial-gradient(ellipse at 50% 40%, rgba(244,163,0,0.18) 0%, transparent 55%), linear-gradient(160deg, #1A0E04 0%, #0E0904 50%, #150B02 100%)',
            }}
          />
        )}

        {/* Big centre play button — only when video exists and paused */}
        {hasVideo && !playing && (
        ...
        {/* Overlay title — shown when no video, or video exists but paused */}
        {(!hasVideo || !playing) && (
          <div className={styles.overlayTitle}>
            ...
            {!hasVideo && (
              <p className={styles.overlayTagline} style={{ marginTop: 14, opacity: 0.6, fontSize: '0.85rem' }}>
      ...
      {/* ── Bottom controls — only when video exists ── */}
      {hasVideo && (
        <footer
